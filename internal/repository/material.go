...
func (m *materialManager) FindBaseByID(id int) (*baseMaterial, error) {
	...
	err := m.db.QueryRow(`
		SELECT id, user_id, username, name,	price
		FROM material WHERE id = ?`, id,
	).Scan(&mat.ID, &mat.UserID, &mat.Username, &mat.Name, &mat.Price)
	if err != nil {
...
func (m *materialManager) AddBase(tx *sql.Tx, mat mat.Base) error {
	_, err := tx.Exec(`
		INSERT INTO material(user_id, username, name, price) VALUES (?, ?, ?, ?)`,
		mat.UserID, mat.Username, mat.Name, mat.Price)
	if err != nil {
