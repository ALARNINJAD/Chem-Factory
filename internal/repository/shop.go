...
type shopManager struct{ db *sql.DB }

func NewShopManager(db *sql.DB) *shopManager { return &shopManager{db: db} }

func (s *shopManager) EmptyStruct() *shop { return &shop{} }

func (s *shopManager) Export() ([]shop, error) {

	rows, err := s.db.Query("SELECT * FROM shop")
	if err != nil {
...
func (s *shopManager) FindIDByInfo(userID, materialID, price int) (int, error) {
	var id int
	err := s.db.QueryRow("SELECT id FROM shop WHERE user_id = ? AND material_id = ? AND price = ?",
...
func (s *shopManager) FindByInfo(userID, materialID, price int) (*shop, error) {
	var shp shop
	err := s.db.QueryRow(`
...
func (s *shopManager) ReduceNumberByID(tx *sql.Tx, id, number int) error {
...
func (s *shopManager) IncreaseNumberByID(tx *sql.Tx, id, number int) error {
	_, err := tx.Exec("UPDATE shop SET number = number + ? WHERE id = ?", number, id)
	if err != nil {
...
func (s *shopManager) Add(tx *sql.Tx, shp shop) error {
	_, err := tx.Exec(`
...
func (s *shopManager) DeleteByID(tx *sql.Tx, id int) error {
